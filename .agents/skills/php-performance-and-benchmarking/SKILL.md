...
### 4. Hybrid Caching & Multi-Tenant Isolation in PerformanceUtils
...
- **SHA-256 APCu Cache Keys:** To prevent namespace collisions in multi-tenant environments and satisfy SonarCloud security requirements, APCu cache keys inside `PerformanceUtils` are generated dynamically as installation-specific strings using SHA-256 (`hash('sha256', ...)`) of the project root directory, avoiding weak algorithms like MD5.
